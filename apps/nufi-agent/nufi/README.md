...
|---|---|---|
| `src/bundles/lfx-bundles/src/lfx_bundles/ollama/ollama.py` | The Ollama chat-model component the box's routines use | **One `IntInput` and one line in `llm_params`**: `num_predict`. A routine talks to Ollama directly, so LiteLLM's `request_timeout: 600` is not in its path, and the component's own `Timeout` input is dropped on the floor — it is set in `llm_params` and `ChatOllama` has no such field, so pydantic ignores it (verified against the pinned langchain-ollama 0.3.10, which *does* have `num_predict`). Without a ceiling a generation runs until something unloads the model: P2 watched one pass 40,000 tokens, still climbing after the client that asked for it had been killed. The component already exposes `num_ctx`, `num_gpu`, `repeat_last_n` and the mirostat trio, so this is the shape upstream already has and simply never wired — worth sending them as a pull request, and living here until it lands |
| `src/backend/base/langflow/services/auth/service.py` | The JIT sign-in path — `_initialize_jit_user_defaults`, which upstream calls the first time an external identity resolves to a new account | **Five added lines, no deletions**: an import, a comment pointing here, and a call to `nufi/member_routines.py` on *each* of upstream's two SSO paths — the one that creates a profile and the one that finds an existing one. The second was missed at first, and a live box found it: the seeding fired once, on a member's very first sign-in, so everyone who had signed in before the box was upgraded stayed empty forever and "the copies you leave alone follow the box" was true for nobody. The seeder is idempotent, so both paths can call it. A member arriving over SSO becomes a non-superuser account that owns nothing, and a flow is visible to the account that owns it — so without this the NuFi box's four department routines are an admin-only feature, which is what P2 proved live. The seeding itself is in `nufi/`, and only the hook is here: there is no service-plugin seam to hang it on (the service factory has no entry-point mechanism), and seeding from outside the image would mean polling for new accounts and a member seeing an empty Studio on the visit that matters. Kept to three lines so the resync conflict, when it comes, is a three-line merge |
