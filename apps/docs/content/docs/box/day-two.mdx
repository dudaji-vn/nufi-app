...
`support` gathers what an engineer actually needs for the first reply on a
...
few small files (`schedules.ini`, `caddy/mesh.caddy`, the drive tree as names
only, the `data/backup` listing) — into one `.tar.gz` next to `data/support/`
(or wherever `--to DIR` says). It prints the path, the size, and who to send
it to.

Nothing in it is a secret. `env-keys.txt` lists the *names* of every key in
`.env` and whether each has a value, never the value; `compose.yml` is
`docker compose config` with every value that looks like a secret — a `.env`
key ending in `_KEY`, `_SECRET`, `_PASSWORD`, `PEM` or `TOKEN` — blanked out
everywhere it appears. It works on a box that will not start and one whose
`doctor` is failing: a check that cannot answer says so in the bundle instead
