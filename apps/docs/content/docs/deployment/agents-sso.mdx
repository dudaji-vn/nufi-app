...
CHAT_PUBLIC_URL=https://chat.nufi.me   # optional; defaults to CHAT_BASE_URL
```

This is the whole configuration surface of the console's half. Every
variable, what it decides, and what happens if you leave it out:
...
`AGENT_ENTITLEMENTS` decides, and the console enforces it at all three
doors a member can arrive through: `GET /enter/studio`, `GET
/oidc/authorize`, and `GET /enter/products`, which the chooser asks
before it renders so a member sees a card they cannot click with a
reason on it, rather than discovering the refusal by clicking into it.

Unset, both products are open to every member — the same behaviour as
before there was a gate, so turning this on is a decision you make
rather than one a deploy makes for you. A malformed value closes both
products to everyone except a NUFI administrator; an administrator is
always entitled, so a typo in one Railway variable cannot lock out the
