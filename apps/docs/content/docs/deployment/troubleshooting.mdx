...
- **`SCANNER_API_BASE` unreachable** — the injection classifier took
  longer than expected to come up (first boot downloads a ~700 MB DeBERTa
  model). Wait, or raise `start_period` on its healthcheck.
...
version. See [Upgrade chat backend](/docs/deployment/upgrade-librechat).
