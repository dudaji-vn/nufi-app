...
check "the chooser responds"       "$(code "$AGENTS/")"   "200"
check "console publishes JWKS"     "$(code "$CONSOLE/.well-known/jwks.json")" "200"

echo
echo "== white label =="
check "studio names no upstream"   "$(count langflow "$(body "$STUDIO/")")"  "0"
check "works names no upstream"    "$(count paperclip "$(body "$WORKS/")")"  "0"
