...
배경. 1차 측정(2026-07-29, gemini-2.5-flash, 프롬프트 1종, n=6)에서 ⟦E1⟧ 0/6,
[[E1]] 2/6, <E1> 6/6 이 나왔으나, 2차 측정(2026-07-30, 같은 모델, 프롬프트 7종,
델리미터 3종)에서 54/54 전부 그대로 반환되어 1차를 재현하지 못했다. 즉 특정
델리미터를 결함으로 단정할 수는 없다.

그럼에도 설정 가능해야 하는 이유는 남는다. 토큰이 그대로 돌아오는지는 라이브러리의
속성이 아니라 모델과 프롬프트의 속성이고, 라이브러리는 배포처의 모델을 미리 알 수
없다. 그리고 실패했을 때가 문제다 — ⟦E1⟧ → E1 처럼 괄호가 벗겨지면 _LENIENT 는
양쪽 괄호를 요구하므로 매칭되지 않아 복원이 조용히 실패한다.
"""
import importlib
import os
import subprocess
