...
### Fixed
- **LLM 이 기본 델리미터를 제거해 복원이 조용히 실패하던 문제** — `gemini-2.5-flash` 측정(같은 프롬프트 n=6, temperature 1.0): 모델이 토큰을 그대로 되돌려주는 비율이 `⟦E1⟧` **0/6**, `[[E1]]` 2/6, `<E1>` **6/6**. `⟦E1⟧ → E1` 이 되면 `_LENIENT` 는 양쪽 괄호를 요구하므로 매칭되지 않아, 사용자가 원본 대신 `E1` 을 보게 된다. 실제 왕복 검증에서 기본값은 0/6, `NUFI_SURROGATE_DELIMS='<,>'` 는 6/6 복원.
- `MAX_SURROGATE_LEN` 이 16 으로 고정되어 있어 긴 델리미터에서 미완결 토큰이 상한을 넘길 수 있던 문제 — 델리미터 길이를 반영하도록 수정.

> `_LENIENT` 는 의도적으로 넓히지 않았다. 맨 `E1`/`P1`/`T2` 는 셀 참조·부품번호 같은 평범한 문자열이라, 괄호 없이 매칭하면 정상 텍스트를 손상시킨다. n=6 은 보장이 아니라 "6회 중 실패 없음"이므로, 호출측은 복원 실패를 별도로 탐지해야 한다.
