...
- **Two checks left in the acceptance scripts that cannot fail, and one wait that can flake.** P2 found and fixed four checks that could not fail — the citation gate, the ingest gate, `drive-write` and `box-on-mesh` — and three of those only because the first one prompted a sweep of its siblings. A check that cannot fail is worse than no check: it reports a pass nobody earned. Two are still open:
  - `deploy/box/tests/vm/verify-ubuntu-box.sh:40` gates the banner on `sed -n "/is up\./,/Day two/p"`, and `sed -n` exits 0 when its range matches nothing, so only a *missing* `install.log` trips the `|| die`. An install that failed reads as one that reached the banner.
  - `deploy/coordinator/lab/day-at-home.sh`'s `online` gate — the fix for `box-on-mesh`, which used to pass on a registration the lab keeps between runs — is a single shot, while every other wait in that file is a bounded loop. A second's lag between tailscaled holding an address and headscale's CLI reporting `online` costs a VM boot. Three attempts two seconds apart; four lines.
