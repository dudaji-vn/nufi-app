...
- **The acceptance scripts' last two gaps are closed.** P2 found and fixed four checks that could not fail — the citation gate, the ingest gate, `drive-write` and `box-on-mesh` — and three of those only because the first prompted a sweep of its siblings. The two that survived that sweep are now closed too: `verify-ubuntu-box.sh` gates the banner on the output rather than on `sed -n`'s exit status, which is 0 for a range that matches nothing, so an install that died before the banner no longer reads as one that reached it; and `day-at-home.sh`'s `online` gate waits in a bounded loop like every other wait in that script, so a second's lag between tailscaled holding an address and headscale reporting `online` no longer costs a VM boot.
